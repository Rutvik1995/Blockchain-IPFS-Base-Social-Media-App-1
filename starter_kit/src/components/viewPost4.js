      ...
      ipfs.files.read("/user/"+userInformationListFromBlockChain.userId+"/userInformationTable",(error,result)=> {
       // console.log(result[0]);
         ...
         if(this.state.postViewerName.includes(userJsonResult.firstName)){
            ...
            //   SecretId: secretName,
            // }).promise();
            this.getAwsSecretAsync(userJsonResult.userId);
            ...
            //   this.setState({})
            // });
            // var response = this.getAwsSecret(userJsonResult.userId).catch(err => (error = err));
            // console.log(this.getAwsSecretAsync);
            
            
                // this.setState({postViewPrivateKey:response.SecretString});
  ...
  //group Information 
      ...
      pausecomp=(millis)=>{
       ...
       async getAwsSecretAsync(secretName) {
        var response = await client.getSecretValue({ SecretId: secretName }).promise();
      ...
      //  return client.getSecretValue({ SecretId: secretName }).promise();
       ...
       // this.pausecomp(200);
        ...
        ipfs2.get("/ipfs/"+IPFSHash,(error,result)=>{
          ...
          this.setState({photoSet:data.image});
          this.setState({videoSet:data.video});
         ...
         // this.setState({postPhotoVideoData:data.imageVideoIPFSHash});
          ...
          var digitalSignature=postInformation.digitalSignature;
          console.log(digitalSignature);
          // var hash1=CryptoJS.SHA256(textualData).toString();
          // bytes  = CryptoJS.AES.decrypt(postViewPrivateKey, digitalSignature);
          // var hash2 = bytes.toString(CryptoJS.enc.Utf8);
          // console.log("hash 1");
          // console.log(hash1);
          // console.log("hash 2");
          // console.log(hash2);
...
<span style={signature}>{this.state.signatureText}</span>
                  ...
                  <div className="container" >
                  <img src={this.state.postPhotoVideoData} style={{height:'400px', width:"100%"}} alt="Notebook" />
                  <div style={text}>
        ...
        return(
            <div className="container">
              <div className="container">
              <h2 style={ReactHeading}>Hello {this.state.postViewerName}</h2>
                <br></br>
                <br></br>
                {renderAuthButton2()}
                <br>
